var AnnouncementController = function () {
    ...
    function registerEvents() {
        ...
        $('body').on('click', '.btn-view', function (e) {
            ...
            $.ajax({
                type: "GET",
                url: "/Admin/Announcement/GetById",
                data: { id: id },
                dataType: "json",
                beforeSend: function () {
                    tedu.startLoading();
                ...
                success: function (response) {
                    var data = response;
                    $('#hidIdTB').val(data.Id);
                    $('#txtNameTB').val(data.Title);
                    $('#txtTimeTB').val(tedu.dateTimeFormatJson(data.DateModified));
                    $('#txtContentTB').val(data.Content);
        ...
        $('#btnSaveM').on('click', function (e) {
            if ($('#frmMaintainance').valid()) {
                ...
                var userid = $('#hidUserIdM').val();
                var content = CKEDITOR.instances.txtContentM.getData();
                var status = $('#ckStatusM').prop('checked') === true ? 1 : 0;

                $.ajax({
                    type: "POST",
                    url: "/Admin/Announcement/SaveEntity",
                    data: {
                        Id: id,
                        Title: title,
                        Content: content,
                ...
                if (response.RowCount > 0) {
                    $.each(response.Results, function (i, item) {
                        render += Mustache.render(template, {
                            ...
                            Id: item.Id,
                            DateModified: tedu.dateTimeFormatJson(item.DateModified),
