...
namespace ThanhThoiApp.Application.Implementation
{
    public class RoleService : IRoleService
    {
        ...
        public List<FunctionViewModel> GetListFunctionWithRole(string[] roles)
        {
            var functions = _functionRepository.FindAll();
            var permissions = _permissionRepository.FindAll();
            //Chỉ l^^ấy ra những thằng function n``ao có permission l``a được đọc = true
            var query = (from f in functions
                         join p in permissions on f.Id equals p.FunctionId into fp
                         from p in fp.DefaultIfEmpty()
                         join r in _roleManager.Roles on p.RoleId equals r.Id
                         where p != null && roles.Contains(r.Name)
                         where p.CanRead == true
                         select f);
            return query.ProjectTo<FunctionViewModel>().ToList();
        }

        //public List<FunctionViewModel> GetListFunctionWithRole(Guid roleId)
        //{
        //    var functions = _functionRepository.FindAll();
        //    var permissions = _permissionRepository.FindAll();
        //    //Chỉ l^^ấy ra những thằng function n``ao có permission l``a được đọc = true
