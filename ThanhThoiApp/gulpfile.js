...
paths.clientJsSide = paths.webroot + "client-side/**/*.js";
...
gulp.task("clean:clientJsSide", function (cb) {
    rimraf(paths.concatJsClientDest, cb);
});
...
gulp.task("clean", ["clean:adminJs", "clean:clientJs", "clean:clientJsSide", "clean:adminCss", "clean:clientCss"]);
...
gulp.task("min:clientJsSide", function () {
    return gulp.src([paths.clientJsSide, "!" + paths.minJs], { base: "." })
        .pipe(concat(paths.concatJsClientDest))
        .pipe(uglify())
        .pipe(gulp.dest("."));
});
...
gulp.task("min", ["min:adminJs", "min:clientJs", "min:clientJsSide",  "min:adminCss", "min:clientCss"]);