...
namespace ThanhThoiApp.Application.Implementation
{
    public class FunctionService : IFunctionService
    {
        ...
        //public Task<List<PermissionViewModel>> GetAllByPermission(Guid userId)
        //{
        //    var listfunction = _permissionRepository.FindAll(x => x.RoleId == userId);
        //    var query = _permissionRepository.FindAll(x => x.RoleId == userId);
        //    return query.OrderBy(x => x.Function.ParentId).ProjectTo<PermissionViewModel>().ToListAsync();
        //}
        public List<FunctionViewModel> GetAll()
        {
            return _functionRepository.FindAll().OrderBy(x => x.ParentId)
        ...
        public PagedResult<FunctionViewModel> GetAllPaging(string keyword, int page, int pageSize)
        {
            ...
            query = query.OrderByDescending(x => x.ParentId)
                .Skip((page - 1) * pageSize).Take(pageSize);

            var data = query.ProjectTo<FunctionViewModel>().ToList();
        ...
        public void ReOrder(string sourceId, string targetId)
        {
            ...
            string tempParent = source.ParentId;

            source.SortOrder = target.SortOrder;
            target.SortOrder = tempOrder;
            source.ParentId = target.ParentId;
            target.ParentId = tempParent;
