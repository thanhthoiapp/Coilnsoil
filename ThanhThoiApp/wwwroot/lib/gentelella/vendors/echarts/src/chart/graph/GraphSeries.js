define(function (require) {
    ...
    var GraphSeries = require('../../echarts').extendSeriesModel({
        ...
        getInitialData: function (option, ecModel) {
            ...
            function beforeLink(nodeData, edgeData) {
                ...
                nodeData.wrapMethod('getItemModel', function (model) {
                    var CategoryModels = self._CategoryModels;
                    var categoryIdx = model.getShallow('category');
                    var categoryModel = CategoryModels[categoryIdx];
                    if (categoryModel) {
        ...
        _updateCategoryData: function () {
            var Category = zrUtil.map(this.option.Category || [], function (category) {
                // Data must has value
                return category.value != null ? category : zrUtil.extend({
                    value: 0
                }, category);
            });
            var CategoryData = new List(['value'], this);
            CategoryData.initData(Category);

            this._CategoryData = CategoryData;

            this._CategoryModels = CategoryData.mapArray(function (idx) {
                return CategoryData.getItemModel(idx, true);
            });
        },
